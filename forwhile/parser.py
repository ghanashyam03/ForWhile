...
def p_error(p):
    ...
    if p:
        # Check if lineno is available on the token
        line = getattr(p, 'lineno', None)
        raise ForWhileSyntaxError(f"I got stuck when reading '{p.value}' on line {line}. Did you make a typo?", line=line)
    else:
        raise ForWhileSyntaxError("I got stuck at the end of the program. Did you forget to close a block or end a statement?")
